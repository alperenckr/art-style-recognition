...
def relu_forward(X):
    """
    ileri beslemeli hesapta nöronların çıkışına relu uygulanması
    """
    out = np.maximum(X, 0)
    cache = X
    return out, cache
...
def relu_backward(dout, cache):
    """
    Relu fonksiyonunun türeviyle işlem yapılır. 
    Backpropagation sırasında gerekli
    """
    dX = dout.copy()
    dX[cache <= 0] = 0
    return dX
...
def sigmoid_forward(X):
    """
    Çıkış aktivasyonu sigmoid olan nöronların çıkışlarına sigmoid fonksiyonunun uygulanması
    """
    out = utils.sigmoid(X)
    cache = out
    return out, cache
...
def sigmoid_backward(dout, cache):
    """
    Backpropagation işlemi sırasında bir sonraki katmandan geri gelen değerler sigmoid fonksiyonunun türevinden geçerler
    """
...
def tanh_backward(dout, cache):
    """
    Backpropagation sırasında bir sonraki katmandan geri gelen değerler hiperbolik tanjant fonksiyonunun türevinden geçerler
    """
    dX = (1 - cache**2) * dout
    return dX
...
def dropout_backward(dout, cache):
    """
    Dropout katmanının backpropationda kullanımı
    """
    dX = dout * cache
    return dX
